...
<template>
  <div>
    <div class="mb-6">
      <h2 class="font-display text-2xl font-semibold">Pre-Authorization</h2>
      <p class="text-muted text-sm mt-1">
        Request approval before inpatient admission or a scheduled procedure.
      </p>
    </div>

    <p v-if="submitted" class="bg-success-soft text-success text-sm rounded-[7px] px-4 py-3 mb-5">
      Request captured. This is a preview form — nothing was sent yet; pre-authorization goes live
    ...
    >
      ...
      <div class="flex flex-col gap-1.5 col-span-2">
        <label class="text-xs font-semibold text-muted">Treatment plan</label>
        <textarea
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-semibold text-muted">Expected admission date</label>
        <input
      ...
      <div class="flex flex-col gap-1.5">
        <label class="text-xs font-semibold text-muted">Expected discharge date</label>
        <input
      ...
      <div class="flex flex-col gap-1.5 col-span-2">
        <label class="text-xs font-semibold text-muted">Estimated amount (KES)</label>
        <input
          v-model="form.amount"
          type="number"
          min="0"
    ...
    <ul class="text-muted text-sm mt-5 list-disc pl-5 space-y-1">
      <li>Pre-authorization is required for all inpatient admissions.</li>
      <li>Request must be submitted at least 24 hours before admission (except emergencies).</li>
      <li>Include a detailed treatment plan and cost estimates.</li>
    </ul>
  </div>
</template>
